...
* **Full Project Generation:** Creates a complete C++ project with:
    ...
    * A 64-bit compatible `proxy.asm` file for function forwarding.
    * A clean `dllmain.cpp` entry point where you can add your custom code.
...
3.  Proximo will load all DLLs used by that process. Select a **non-system (white-colored) DLL** from the list on the left.
...
2.  Download the dependencies. Place `json.hpp` in the project's root source directory.
